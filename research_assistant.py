...
def main():
    ...
    if fetch_button:
        with st.spinner("Fetching data..."):
            ...
            combined_df = pd.concat([df_open_alexs, df_crossrefs], ignore_index=True)
            combined_df['Citations_Total'] = combined_df.get('Citations_OpenAlex', 0) + combined_df.get('Citations', 0)
            top_10_combined = combined_df.sort_values('Citations_Total', ascending=False).head(10)

        tab1, tab2, tab3, tab4 = st.tabs([translate('results'), translate('visualizations'), translate('archive'), 'Consolidated Top 10'])

        with tab1:
            research_paper_tabs = st.tabs(['OpenAlex Results', 'CrossRef Results'])
...
def process_open_alexs(data):
    ...
    for entry in data.get('results', []):
        authors = ', '.join([a["author"]["display_name"] for a in entry.get("authorships", []) if "author" in a])
        institutions = ', '.join([inst.get("display_name", "Unknown")
        ...
        paper = {
            ...
            'Citations_OpenAlex': entry.get("cited_by_count", 0),
...
def process_crossrefs(data):
    ...
    if 'message' in data and 'items' in data['message']:
        for entry in data['message']['items']:
            authors = ', '.join(f"{a.get('family', '')}, {a.get('given', '')}" for a in entry.get('author', []))
            paper = {
                'Title': entry.get('title', ['N/A'])[0],
                'Authors': authors,
                'Institution': 'N/A',
                'Year': entry.get('published-print', {}).get('date-parts', [['N/A']])[0][0]
                        if entry.get('published-print') else 'N/A',
...
def create_visualizations_openalex(df):
    ...
    st.plotly_chart(px.bar(yearly_publications, x='Year', y='Count', title='Yearly Publications'))
    ...
    st.plotly_chart(px.line(yearly_citations, x='Year', y='Citations_OpenAlex', title='Citations Per Year'))
...
def create_visualizations_crossref(df):
    ...
    yearly_citations['Year'] = pd.to_numeric(yearly_citations['Year'], errors='coerce')
    st.plotly_chart(px.line(yearly_citations, x='Year', y='Citations', title='Citations Per Year'))
